...
export const generatePDF = async (data: FormData): Promise<void> => {
  ...
  // Header - Logo (paceias_page-0001.jpg)
  let logoLoaded = false;
  try {
    const response = await fetch('/paceias_page-0001.jpg');
    ...
    pdf.addImage(base64, 'JPEG', leftMargin, y - 5, 53, 14);
    logoLoaded = true;
  } catch {
    logoLoaded = false;
  }
  ...
  addField('Home Institution', data.homeInstitution, leftMargin, y, 35);
  y += 7;
  ...
  pdf.text(introText1, leftMargin, y); y += 4;
  pdf.text(introText2, leftMargin, y); y += 5;
  pdf.text('• Fall courses are posted online in mid to late April.', leftMargin + 2, y); y += 4;
  pdf.text('• Spring courses are posted online in mid to late November.', leftMargin + 2, y); y += 5;
  ...
  pdf.text(introText3, leftMargin, y); y += 4;
  pdf.text(introText4, leftMargin, y); y += 4;
  pdf.text(introText5, leftMargin, y); y += 4;
  pdf.text(introText6, leftMargin, y); y += 4;
  pdf.text(introText7, leftMargin, y); y += 8;
  ...
  // Acknowledgement — add new page if not enough room
  checkPageBreak(55);
  pdf.setFontSize(9);
  pdf.setFont('helvetica', 'bold');
  pdf.text('Acknowledgement', leftMargin, y);
  y += 5;
  pdf.setFont('helvetica', 'normal');
  pdf.setFontSize(8);
  const ackText = 'By signing this form, you acknowledge that you are solely responsible for selecting your courses and for confirming with your home institution';
  const ackText2 = 'whether these courses will transfer toward your degree upon completion of your exchange semester(s). You also understand that submitting';
  const ackText3 = 'an incomplete form or submitting the form after the stated deadline may result in gaps in your schedule, require you to resubmit this form';
  const ackText4 = 'multiple times with updated course selections, and may ultimately require you to enroll in courses that do not count toward your degree in';
  const ackText5 = 'order to maintain your full-time visa enrollment requirement as classes fill up.';
  pdf.text(ackText, leftMargin, y); y += 4;
  pdf.text(ackText2, leftMargin, y); y += 4;
  pdf.text(ackText3, leftMargin, y); y += 4;
  pdf.text(ackText4, leftMargin, y); y += 4;
  pdf.text(ackText5, leftMargin, y);
  y += 10;

  // Signature line
  pdf.setFontSize(9);
  pdf.text('Signature:', leftMargin, y);

  if (data.signatureType === 'pad' && data.signatureData) {
    try {
      pdf.addImage(data.signatureData, 'PNG', leftMargin + 25, y - 8, 50, 20);
    } catch {
      // Signature failed
    }
  }
